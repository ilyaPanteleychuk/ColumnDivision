...
public class ClassicFormatter implements Formatter {
    ...
    public void drawHeader(Result result){
        Divider.Step firstStep = result.getStepsList().get(0);
    ...
    private void drawStep(List<Divider.Step> steps){
       if(!(steps.isEmpty())){
           if(getIntLength(steps.get(0).getINTEGRAL_PARTIAL_DIVIDEND()) == 1){
               output.append(WHITE_SPACE)
                   .append(drawIndent(steps.get(0).getPOSITION() + 1))
               ...
               output.append(WHITE_SPACE)
                   ...
                   .append(steps.get(0).getINTEGRAL_PARTIAL_DIVIDEND())
                   .append(lineSeparator());
               if (steps.get(0) != steps.get(steps.size() - 1)) {
                   output.append(WHITE_SPACE)
                       .append(drawIndent(steps.get(0).getPOSITION()))
